  ...
  return (
    <section className="rounded-[2.5rem] border border-slate-200/60 bg-white/70 backdrop-blur-md py-12 md:py-16 px-6 md:px-10 space-y-10 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]">
      <div className="flex flex-col items-center justify-center gap-4 text-center">
        <div className="inline-flex items-center gap-2 rounded-full bg-success-500/10 px-4 py-2 text-sm font-bold uppercase tracking-widest text-success-500">
        ...
        <p className="text-base md:text-lg font-medium text-muted-foreground max-w-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {printOptions.map((item) => (
          <div
            key={item.title}
            className="rounded-3xl border border-slate-200/60 bg-white/50 px-6 py-6 text-left transition-all duration-300 hover:border-success-500/30 hover:bg-white hover:shadow-md hover:-translate-y-1 flex flex-col items-start"
          >
            <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-sm border border-border/60 text-success-500">
